...
class RewardMachine:
    def __init__(self, file):
        ...
        self.terminals = set() ## NOTE: Required to support multiple terminal states
    ...
    def __str__(self):
        """
        Returns a displayable version of the reward machine
        """
        return f"Initial State: {self.u0}\nNon-Terminal States: {self.U}\nTerminal State: {self.terminal_u}\nState Transition: {self.delta_u}\nReward Transition: {self.delta_r}\n"
    ...
    def count_states(self):
        return len(self.U) + len(self.terminals)

    def count_transitions(self):
        total = 0
        for state, edges in self.delta_u.items():
            total += len(edges)
        return total
    ...
    def eval_sequence(self, sequence):
        ...
        for prop in sequence:
            u, rew, done = self.step(u, prop, None)
            if done:
                return rew
            last_rew = rew
        return last_rew
    ...
    def step(self, u1, true_props, s_info, add_rs=False, env_done=False):
        ...
        assert u1 != self.terminal_u and u1 not in self.terminals, "the RM was set to a terminal state!"
        u2 = self.get_next_state(u1, true_props)
        done = (u2 == self.terminal_u or u2 in self.terminals)
        # Getting the reward
    ...
    def _load_reward_machine(self, file):
        ...
        self.terminals = set(terminal_states)
        # adding transitions
        for e in lines[2:]:
            # Reading the transition
            u1, u2, dnf_formula, reward_function = eval(e)
            # terminal states
            if u1 in self.terminals:
                continue
            #if u2 in self.terminals:
            #    u2  = self.terminal_u
    ...
    def _add_state(self, u_list):
        for u in u_list:
            if u not in self.U and (u != self.terminal_u and u not in self.terminals):
                self.U.append(u)
