...
for j in `seq 0 99`;
do
for i in `seq 105 110`;
do
    sem --id office --bg --jobs 85 "nohup python experiment.py --eq_samples 2000 --ids_sampling --domain craft --task t$i --trial $j  1> /dev/null 2> logs/sophis-craft_world_abr_2000_100_t$i.err.$j"
done
done
