...
do
	# Single task
    sem --id office --bg --jobs 10 "nohup python run.py --env_kwargs sample=$SAMP --alg=qlearning --env=Office-single-T$j-v0 --num_timesteps=2e5 --gamma=0.9 --log_path=../my_results/crm/office-single/M1/2e5-t$j-$SAMP/$i --use_crm 1> /dev/null 2> office_t$j.err.$SAMP.$i"
