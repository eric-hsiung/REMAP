...
## Create a parameterizable REFERENCE environment for a single map, single task
class CraftRefRMEnv(GridRMEnv):
    def __init__(self, map_id=0, task_id=1):
        ## Use a specific map, based on the map ID
        file_map = f"./envs/grids/maps/map_{map_id}.txt"
        ## Solve a specific task, based on the task ID
        rm_files = [f"./envs/grids/reward_machines/craft/t{task_id}.txt"]
        env = CraftWorld(file_map)
        super().__init__(GridEnv(env), rm_files)

## Create a parameterizable LEARNED environment for a single map, single task, sample setting
class CraftLearnedRMEnv(GridRMEnv):
    def __init__(self, map_id=0, task_id=1, sample=200):
        ## Use a specific map, based on the map ID
        file_map = f"./envs/grids/maps/map_{map_id}.txt"
        ## Solve a specific task, based on the task ID and sample setting
        rm_files = [f"./lstar_exps/reward_machine_experiments/craft-{sample}/t{task_id}.txt.{i}" for i in range(0,100)]
        env = CraftWorld(file_map)
        super().__init__(GridEnv(env), rm_files)

## Create a parameterizable LEARNED environment for a single map, single task, sample setting
class CraftLearnedSingleRMEnv(GridRMEnv):
    def __init__(self, map_id=0, task_id=1, sample=200, trial=0):
        ## Use a specific map, based on the map ID
        file_map = f"./envs/grids/maps/map_{map_id}.txt"
        ## Solve a specific task, based on the task ID and sample setting
        rm_files = [f"./lstar_exps/reward_machine_experiments/craft-{sample}/t{task_id}.txt.{trial}"]
        env = CraftWorld(file_map)
        super().__init__(GridEnv(env), rm_files)
